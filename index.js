...
Usa ${prefix}verify para registrarte y poder usar el bot.`
  ...
  only: {
    ...
    daftarB: `[◇]Hola!, Registrate usando *${prefix}verify* para poder usar el bot owo`
  }
}
...
case 'chico':
  ...
  Lxa.updatePresence(from, Presence.composing)
  uk = ["chico lindo", "chico genial", "chico guapo", "chico", "chico sexy"]
  nk = uk[Math.floor(Math.random() * uk.length)]
  try {
  data = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${nk}`, {
method: 'get'
...
quoted: mek
  ...
  if (!q.includes('-')) return  reply('[◇]Ingrese la fecha-mes-año correctamente \ n *Ejemplo: 09-09-2009*)
  pc = body.slice(7)
  teks1 = pc.split("-")[0];
  teks2 = pc.split("-")[1];
  teks3 = pc.split("-")[2];
...
Lxa.sendMessage(from, `‣ *[◇]Demon al azar:*\n${der}`, text, {
  quoted: mek
})
break
