...
const {
  ...
  + 'FN:♥️JK変態BSUwU🖤\n' // Nama
  + 'ORG:〘👻●@bsuwu●🇦🇷〙;\n' // Nama bot
  + 'TEL;type=CELL;type=VOICE;waid=17472081051:+1 (747) 208-1051n' // Nomor bot
  + 'END:VCARD' 
  ...
  */
  ...
  async function starts() {
	  ...
	  Lxa.on('qr', () => {
		  console.log(color('[','white'), color('!','red'), color(']','white'), color('[◇] Ecaneá el código, pero rápido flaco, dale'))
	  })
...
⊰᯽⊱┈─────╌❊ - ❊╌─────┈⊰᯽⊱
  ...
  ferr: '[◇] ⚠️ Disculpá flaco, hubo un error.',
  limitend: '[◇] El tiempo de carga se acabó lcdth.',
  error: {
  stick: '[◇] Intentálo después bldo.',
  Iv: '[◇] ⚠️ Hubo un error en el link boludo.'
  },
  only: {
    ...
    daftarB: `[◇] Hola!, Registráte usando *${prefix}verificar* Para poder usar el bot qwq`
  }
}
  ...
  */
  ...
  //--Auto respon
  if(budy.match('Benja')){
  result = fs.readFileSync(`./temp/stick/uwu.webp`)
	Lxa.sendMessage(from, result, sticker, {
  quoted: mek
	})
  }
    if(budy.match('Benkook')){
  ...
  switch(is) {
	case 'Benja':
  buf = fs.readFileSync(`./temp/audio/Ara ara.mp3`)
...
┃‣ _*Prefijo* :  *「  ${prefix}  」*_
┃───────────────────
┃‣ _*Conección* : *Patata kawai ahre*_
┃───────────────────
...
┗━━━━━━━━━━━━━━━━━━━━
 ...
 ┃ _Mencionarte_
 ┠━━━━━━━━━━━━━━━━━━ 
 ┠⊷️❥ *${prefix}Fitnah (Menciona persona|txt1|txt2)*
...
┗━━━━━━━━━━━━━━━━━━━━
 ...
 ┠⊷❥ *${prefix}Verdad*
 ┠━━━━━━━━━━━━━━━━━━ 
 ┃creo que estan mal xd
 ┗━━━━━━━━━━━━━━━━━━

┏━━━━━━━━━━━━━━━━━━━━
● *〘CHOTA MAGICA〙* xd ahre no
┗━━━━━━━━━━━━━━━━━━━━
 ┠⊷❥ *${prefix}Apakah (Texto)*
 ┠━━━━━━━━━━━━━━━━━━ 
...
┗━━━━━━━━━━━━━━━━━━━━
 ┠⊷️❥ *${prefix}Tomp3*
 ┃ _Sacar un audio de un video_
 ┠━━━━━━━━━━━━━━━━━━ 
 ┠⊷️❥ *${prefix}Play (Nombre)*
 ┃ _Canciones y descarga_
 ┠━━━━━━━━━━━━━━━━━━ 
 ┠⊷️❥ *${prefix}Aud(Texto)*
 ┃ _Texto a sonido de google_
 ┠━━━━━━━━━━━━━━━━━━ 
  ...
  */
	  ...
	  break
				  ...
				  case 'bass':                 
					  ...
					  break
 case '17#0':
	teks = `Puto el que lee`
  //--Info bot
				  case 'info':
	me = Lxa.user
	uptime = process.uptime()
	teks = `──────────────────
_‣ Nombre del bot* : ${me.name}_
 ──────────────────
  _‣ *Número del bot* : @${me.jid.split('@')[0]}_
   ──────────────────
  _‣ *Dueño* : ♥️JK変態BSUwU🖤_
   ──────────────────
  _‣ *Prefijo* :   ${prefix}_
  ...
  _‣ *Versión* : V.0.4.5_
   ──────────────────
  _‣ *Usuarios bloqueados* : ${blocked.length}_
   ──────────────────
  _‣ *Tiempo activo* : ${kyun(uptime)}_
   ──────────────────
  _‣ *Total de usuarios* : ${_registered.length} Usuarios_
   ──────────────────
  _‣ *Total de chats* : ${totalchat.length}_
   ──────────────────`
...
‣ *Negativo* : ${data.negatif}
 ──────────────────`
  ...
  case 'gay%':
	if (!isRegister) return reply(mess.only.daftarB)
	if (args.length < 1) return Lxa.sendMessage(from, '[◇] Escribí el nombre', text, {
  quoted: mek
	})
	teks = body.slice(13)
	Lxa.updatePresence(from, Presence.composing)
  ...
  //--searching lirik
  case 'lirik':
	if (args.length < 1) return reply('[◇] Escribí el nombre de la cancion')
	if (!isRegister) return reply(mess.only.daftarB)
  ...
  */
					...
					case 's':
					  if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						  ...
						  await ffmpeg(`./${media}`)
							  ...
							  .on('end', function () {
								  console.log('Finish')
								  exec(`webpmux -set exif ${addMetadata('idk Jsjd', '@bsuwu')} ${ran} -o ${ran}`, async (error) => {
						  ...
						  await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							  ...
							  fs.writeFileSync(ranp, buffer, (err) => {
								  if (err) return reply('[◇] ⚠️ Se produjo un error, Volvé a intentarlo más tarde flaco.')
							  })
							  exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								  fs.unlinkSync(ranp)
								  if (err) return reply(mess.error.stick)
								  exec(`webpmux -set exif ${addMetadata('idk Jsjs', '@bsuwu')} ${ranw} -o ${ranw}`, async (error) => {
									  if (error) return reply(mess.error.stick)
					  ...
					  /*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						  ...
						  reply(`[◇] Envíá una imagen con el comando  *${prefix}sticker*  o respondé a una imagen que se asha enviado xd`)
					  }
					  break
  ...
  //-- temp
			  ...
			  case 'slist':
				  ...
				  teks += `\n*Total : ${setiker.length}*\nUsa el comando\n*${prefix}versticker (nombre del pack)*\npara seleccionar sticker`
			  ...
			  case 'addvn':
			   case 'adudio+':
				  ...
				  Lxa.sendMessage(from, `[◇] Audio añadido correctamente\ncompruebalo con: ${prefix}Audiolist`, MessageType.text, { quoted: mek })
				  break
			  ...
			  case 'addimg':
				  ...
				  Lxa.sendMessage(from, `[◇] Imagen agregada\nCompruebalo con${prefix}Imglist`, MessageType.text, { quoted: mek })
				  break
			  ...
			  case 'imglist':
				  ...
				  teks += `\n*Total : ${imagenye.length}*\nUsa el comando\n*${prefix}Verimg(nombre de la imagen)*\n`
  ...
  //--google voice
				  case 'tts':
			      case 'aud':
					  if (args.length < 1) return Lxa.sendMessage(from, '[◇] Escribí el código', text, {quoted: mek})
					  const gtts = require('./lib/gtts')(args[0])
					  if (args.length < 2) return Lxa.sendMessage(from, '[◇] Escribí el texto', text, {quoted: mek})
					  dtt = body.slice(8)
					  ranm = getRandom('.mp3')
					  dtt.length > 600
  ...
  //--menurunkan jabatan
				  case 'admin-':
					  ...
					  mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					  if (mentioned.length > 1) {
						  teks = '[◇] Admin quitado correctamente\n'
						  for (let _ of mentioned) {
							  teks += `@${_.split('@')[0]}\n`
						  }
						  ...
						  mentions(`[◇] Un admi menos @${mentioned[0].split('@')[0]} por trolo!`, mentioned, true)
  ...
  */
  ...
  //--Convert stiker to image
				  ...
				  case 'aimagen':
					  ...
					  exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						  ...
						  buffer = fs.readFileSync(ran)
						  Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: '〘👻●@bsuwu●🇦🇷〙'})
						  fs.unlinkSync(ran)
					  })
					  break
  ...
  case 'daftar':
	case 'register':
	case 'verificar':
  if (isRegister) return reply('[◇] Che, tu cuenta ya estaba verificada')
...
⊰᯽⊱┈───╌❊ - ❊╌───┈⊰᯽⊱`
  ...
  if (!isBotGroupAdmins) return reply(mess.only.Badmin)
  const open = {
	text: `[◇] Gupo abierto por @${sender.split("@")[0]}\n ahora todos pueden hablar`,
	contextInfo: {
  mentionedJid: [sender]
	}
  }
