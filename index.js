...
+ 'FN:۝⃢⃟🇦🇷👻𝕭-𝕭suwu👑۝⃢⃟🇦🇷\n' // Nama
+ 'ORG:۝⃢⃟🇦🇷👻𝕭-𝕭suwu👑۝⃢⃟🇦🇷;\n' // Nama bot
+ 'TEL;type=CELL;type=VOICE;waid=17273061364:+1 727 306-1364\n' // Nomor bot
+ 'END:VCARD' 
...
Usa ${prefix}verify para registrarte y poder usar el bot.`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Que bueno que se fúe el gordo pelotudo este @${num.split('@')[0]}, ahora hay mas espacio xd`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
  ...
  only: {
    ...
    ownerG: '[◇] Este comando solo lo puede usar el dueño del grupo',
    ownerB: '[◇] Este comando solo lo puede usar el dueño del bot',
    admin: '[◇] Tenés que ser admin para usar este comando bldo',
    Badmin: '[◇] El bot tiene que ser admin para usar este comando',
    daftarB: `[◇] Hola!, Registráte usando *${prefix}verify* para poder usar el bot owo`
  }
}
...
mimetype: 'stiker/webp', quoted: mek, ptt: true
  })
}
...
case 'menu':
hasil = `        ────────────────
Hola *${pushname}* usá este comando ${prefix}menu
        ────────────────`
reply(hasil)
        break
}
...
‣ _*Prefijo* :  *「  ${prefix}  」*_
─────────────────────
‣ _*Creador* :  ♥️JK変態BSUwU🖤_
─────────────────────
‣ _*Conección* :   *Patata kawai*_
─────────────────────
‣ _*Tiempo encendido* :  *${kyun(uptime)}*_
─────────────────────
‣ _*Modo* :   *ON*_
─────────────────────
‣ _*Versión* :     v0.2.0_
...
‣ _*Grupo* :    *${groupName}*_
─────────────────────
‣ _*Número de grupos* :  *${_registered.length}*_
─────────────────────
‣ _*Número de chats* :    *${totalchat.length}*_
─────────────────────
‣ _*Número del dueño* :  wa.me/+5493814721444_
─────────────────────
‣ *Grupo oficial* :
● _Proximamente xd_
─────────────────────
‣ Mis redes?* :
● _Proximamente ndeah_
─────────────────────
...
┃ _Ex : ${prefix}Ephoto 25 Benkoook_
┠⊷️❥ *${prefix}Textpro list*
┃ _Lista de textpro
┠⊷️❥ *${prefix}Textpro (code + Texto)*
┃ _Ex : ${prefix}Textpro 55 Benkook_
┗━━━━━━━━━━━━━━━━━━━━
 ────────────────
*〘 IMAGENES 〙*
 ────────────────
...
//---comandos nuevos---//
					...
					var porn = gh.split("&")[0];
					var hub = gh.split("&")[1];
					if (args.length < 1) return reply(`[◇] Ejemplo : ${prefix}pornhub Benja & Bot`)
					reply(ind.wait())
				...
				case 'glitchtext':
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`[◇] Ejemplo : ${prefix}glitchtext Benja & Bot`)
					reply(ind.wait())
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      reject(e)
				...
				break
				case 'creador':
                  client.sendMessage(from, {displayname: "Benja", vcard: vcard}, MessageType.contact, { quoted: mek})
...
//function antilink 
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					if (isGroupAdmins) return reply('[◇] Porque sos un admin del grupo, El bot no te baneará')
					client.updatePresence(from, Presence.composing)
					if (messagesC.includes("#izinadmin")) return reply("#izinadmin diterima")
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						reply(`[◇] Enlace Detectado, lo siento ${sender.split("@")[0]} a usted le doy una patada en el orto en 5 segundos`)
						setTimeout( () => {
...
//--arti nama
case 'arti':
if (args.length < 1) return reply('*[◇]Escribí el Texto*')
Lxa.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.daftarB)
teks = body.slice(6)
try {
...
Lxa.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.daftarB)
if (!q.includes(' & ')) return  reply('[◇]Formato de texto incorrecto pa')
...
//---Zodiak harian
case 'zodiak':
if (args.length < 1) return reply('[◇]Escribí el signo zodiaco')
Lxa.updatePresence(from, Presence.composing)
if (!isRegister) return reply(mess.only.daftarB)
teks = body.slice(8)
try {
...
if (args.length < 1) return reply('[◇]Escribí lo que querés buscar')
tels = body.slice(5)
Lxa.updatePresence(from, Presence.composing)
reply(mess.wait)
try {
...
Lxa.updatePresence(from, Presence.composing)
tw = ["Twitter divertido de Argentina",
   "Twitter diario",
   "Subestimar Twitter",
   "tweets divertidos"  ]
...
case 'chico':
  ...
  Lxa.updatePresence(from, Presence.composing)
  uk = ["chico lindo", "chico genial", "anime chicos", "chico", "chico sexy", "chico anime"]
  nk = uk[Math.floor(Math.random() * uk.length)]
  try {
  data = await fetchJson(`https://api.fdci.se/sosmed/rep.php?gambar=${nk}`, {
method: 'get'
...
case 'gay%':
  ...
  if (args.length < 1) return Lxa.sendMessage(from, '[◇]Escribe el nombre del pibe', text, {
quoted: mek
  })
  teks = body.slice(13)
  Lxa.updatePresence(from, Presence.composing)
  data = await fetchJson(`https://arugaz.herokuapp.com/api/howgay`)
  hasil = `*Veamos si es gay...*\n‣ Nombre : *${args[0]}*\n‣ Porcentaje : *${data.persen}%*\n*${data.desc}*`
  reply(hasil)
...
case 'seberapabucin':
  ...
  if (args.length < 1) return Lxa.sendMessage(from, '[◇] Ingresa el nombre del pibe', text, {
quoted: mek
  })
  teks = body.slice(15)
  try {
...
case 'textgen':
  teks = body.slice(9)
  if (args.length < 1) return reply('[◇] Escribí el nombre')
  Lxa.updatePresence(from, Presence.composing)
  if (!isRegister) return reply(mess.only.daftarB)
...
case 'jadwaltv':
  ...
  ch = body.slice(10)
  if (args.length < 1) return reply('[◇]Escribí el nombre del canal')
  Lxa.updatePresence(from, Presence.composing)
  reply(mess.wait)
  try {
...
case 'map':
  ...
  if (args.length < 1) return reply('[◇]Escribí el nombre del área')
  daerah = body.slice(5)
  try {
...
case 'alquran':
  ...
  if (args.length < 1) return reply('[◇]Escribí el número del área 1-114')
...
case 'sholat':
  ...
  if (args.length < 1) return reply('[◇]Escribí el número de área')
  try {
...
case 'play':
  ...
  if (args.length < 1) return reply('[◇] Ecribe el nombre')
  reply(mess.wait)
  play = body.slice(6)
  try {
...
//---textpro
				case 'textpro':
					if (args.length < 1) {
						...
						return reply('[◇]Escribí bien el texto')
					}
					reply(mess.wait)
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falló, al momento de convertir ${tipe} el stiker`)
							})
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Lexa', 'Mrf.zvx')} ${ran} -o ${ran}`, async (error) => {
...
exec(`ffmpeg -i ${anum} ${ran}`, (err) => {
  ...
  Lxa.sendMessage(from, buffer, video, {
quoted: mek, caption: '۝⃢⃟🇦🇷👻𝕭-𝖇𝖘𝖚𝖜𝖚👑۝⃢⃟🇦🇷'
  })
  fs.unlinkSync(ran)
})
break
...
//--google voice
				...
				if (args.length < 1) return client.sendMessage(from, '[◇] Se requiere código de idioma!!', text, {quoted: mek})
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('[◇] El texto es en su mayoría es muy largo')
...
//--Hilih maker
				...
				case 'url2img':
					...
					if (args.length < 1) return reply('[◇]Escribí bien el comando')
				...
				case 'añadir':
				  case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('[◇]No se puede agregar al pibe, probablemente este bloqueado por el bot')
					}
					break
				...
				case 'ban':
				  case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Se baneó al conchudo : @${mentioned[0].split('@')[0]}`, mentioned, true)
						Lxa.groupRemove(from, mentioned)
					}
					break
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						if (err) return reply('[◇] No se pudo hacer F')
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: '〘👻●@𝙱𝚂𝚄𝚠𝚄●🇦🇷〙'})
						fs.unlinkSync(ran)
					})
					break
...
reply(anu.result.artimimpi)
  ...
  reply('[◇]Disculpa flaco, hubo un error')
}
break
...
//--Simsimi talk
				case 'simi':
					...
					if (anu.error) return reply('🔺️[◇]pelotudo, escribí el texto bien')
					reply(anu.jawaban)
					} catch {
					  reply(mess.ferr)
					}
...
case 'daftar':
  case 'registrer':
if (isRegister) return reply('[◇]Disculpa flaco, Tu cuenta ya estaba verificada')
...
┠⊷️❥ *Nᴏᴍʙʀᴇ:* *${namaUser}*
┠━━━━━━━━━━━━━━━━━
┠⊷️❥ *Nᴜ́ᴍᴇʀᴏ:* *${sender.split("@")[0]}*
┠━━━━━━━━━━━━━━━━━
┠⊷️❥ *Cᴏ́ᴅɪɢᴏ:* *${serialUser}*
┠━━━━━━━━━━━━━━━━┓
┗━━━━━"@BSUwU"━━━━━━┛
⊰᯽⊱┈───╌❊ - ❊╌───┈⊰᯽⊱`
reply(hasil)
...
tm = `[◇]Verificación completa, usa  ${prefix}Menu  para ver la lista de comandos`
reply(tm)
break
...
Lxa.deleteMessage(from, {
  ...
  reply('[◇] Solo podés borrar mensájes míos')
}
break
...
break
				...
				case 'bienvenid@':
				  case 'welcome':
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('[◇] Ya esta activo pa')
						welkom.push(from)
						fs.writeFileSync('./data/welkom.json', JSON.stringify(welkom))
						reply('[◇]Se activó correcta.ente')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./data/welkom.json', JSON.stringify(welkom))
						reply('[◇ ]Se desactivó correctamente')
					} else {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('[◇] Objetivo de etiqueta')
  ...
  reply(`⊰᯽⊱┈───╌❊ - ❊╌───┈⊰᯽⊱\n Hola *${pushname}* !!!\nDisculpá flaco, esa poronga:\n *${prefix}${command}*\n creo que no está en mi lista de comandos\n Escribí esto para ver los comandos*${prefix}Menu*\n ⊰᯽⊱┈───╌❊ - ❊╌───┈⊰᯽⊱`)
				}
